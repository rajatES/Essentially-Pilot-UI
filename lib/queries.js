...
export function usePostsInvalidate() {
  ...
  return () => {
    ...
    // The failure feed is its own query (see usePostFailures) — invalidate it
    // alongside, or deleting/re-sending a failed post leaves a stale row behind.
    qc.invalidateQueries({ queryKey: ["post-failures"] });
  };
}

// Every failed delivery in the window, one row per failed channel.
//
// Deliberately NOT part of usePostsData: /api/posts is capped at the 100 most
// recent posts, which is a few days at ES's volume, so older failures could
// never appear in it. This asks post_targets directly. `enabled` is passed in
// so the Posts view only fetches it when the Error tab is actually open.
// `includeCleared` asks for failures an admin has cleared from the feed. They
// are hidden by default but never deleted, and the response always reports how
// many there are — see clearFailures() in posts.service.ts.
export function usePostFailures({ enabled = true, days, includeCleared = false } = {}) {
  return useQuery({
    queryKey: ["post-failures", days ?? null, includeCleared],
    queryFn: () => {
      const params = new URLSearchParams();
      if (days) params.set("days", days);
      if (includeCleared) params.set("includeCleared", "1");
      const qs = params.toString();
      return apiJson(`/api/posts/failures${qs ? `?${qs}` : ""}`);
    },
    enabled: enabled && typeof window !== "undefined" && !!getToken(),
    refetchInterval: LIVE_POLL_MS,
  });
}
...
// Per-post performance rollup (GET /api/insights?days=N, or ?start=&end= for a
// custom range). Metrics come from the platform APIs by post id — no scraper.
// `enabled: false` holds the request back — used while a half-filled custom
// range would otherwise silently fall through to the last-30-days default and
// answer a question nobody asked.
export function useInsights({ days, start, end, enabled = true } = {}) {
  const params = new URLSearchParams();
  if (start && end) { params.set("start", start); params.set("end", end); }
  else params.set("days", String(days || 30));
  const qs = params.toString();
  return useQuery({
    queryKey: ["insights", start && end ? `${start}:${end}` : `days:${days || 30}`],
    queryFn: () => apiJson(`/api/insights?${qs}`),
    enabled: enabled && typeof window !== "undefined" && !!getToken(),
  });
}
